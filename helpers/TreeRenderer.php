...
class TreeRenderer {
    ...
    public static function returnTreeWithCheckbox($single = false) {
        ...
        $dataArray = \app\helpers\Common::buildTree(self::$_FORMDATA, 0, $selected, self::$_PARENT_FIELD, self::$_TITLE_FIELD);
    ...
    public static function retutnTreeNormal() {
    ...
    public static function hiddenSelectedValues($tree_id) {
        $model = Functions::checkIsset('model',self::$_options);
        $attribute = Functions::checkIsset('attribute',self::$_options);
        $name = Functions::checkIsset('name',self::$_options,'selectedAttr');
        if($model && $attribute){
...
//                'checkbox' => true,
                ...
                /* 'persist' => [
                  'cookiePrefix' => 'fancytree-1-' . self::$_FORMID . (($id) ? '' : time()*  rand($min, $max)),
                  'expandLazy' => true,
        ...
        //Natk::prettyPrint($treeViewArray);
